...
describe('POST /api/histoire/:idHistoire/paragraphe/', () => {
    ...
    test('Test of failed paragraph creation as a choice (choice already exists)', async () => {
        ...
        const story = await createStory('La cinquieme petite histoire de clauzond');
        const token = await getToken();
...
describe('GET /api/histoire/:idHistoire/paragraphe/:idParagraphe', () => {
    test('Test of get paragraph', async () => {
        ...
        response = await request(app)
            .get(`/api/histoire/${story.id}/paragraphe/${story.idParagrapheInitial}`)
            .set('Content-Type', 'application/json')
            .set('x-access-token', token)
    ...
    test('Test of failed get paragraph (story does not exist)', async () => {
        ...
        const story = await createStory('2. Test of failed get paragraph (story does not exist)');
        const token = await getToken();

        response = await request(app)
            .get(`/api/histoire/789654789/paragraphe/${story.idParagrapheInitial}`)
            .set('Content-Type', 'application/json')
            .set('x-access-token', token)
        expect(response.statusCode).toBe(status.NOT_FOUND);
    ...
    test('Test of failed get paragraph (paragraph does not exist)', async () => {
        ...
        const story = await createStory('3. Test of failed get paragraph (paragraph does not exist)');
        const token = await getToken();
