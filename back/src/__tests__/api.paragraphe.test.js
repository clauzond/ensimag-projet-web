...
describe('POST /api/histoire/:idHistoire/paragraphe/', () => {
    test('Test of new paragraph creation as a choice', async () => {
        ...
        const titreChoix = (await ChoixTable.findOne({
...
describe('DELETE /api/histoire/:idHistoire/paragraphe/:idParagraphe', () => {
    test('Test of delete paragraph', async () => {
        ...
        response = await request(app)
            .delete(
                `/api/histoire/${story.id}/paragraphe/${idChoice}`
            )
            .set('Content-Type', 'application/json')
            .set('x-access-token', token);
        expect(response.statusCode).toBe(status.OK);
        expect(response.body.message).toBe("Paragraph has been successfully deleted");
    ...
    test('Test of failed delete paragraph (paragraph is initial)', async () => {
        ...
        const story = await createStory('2. Test of failed delete paragraph (paragraph is initial)');
        const token = await getToken();
        ...
        response = await request(app)
            ...
            .set('x-access-token', token);
        expect(response.statusCode).toBe(status.BAD_REQUEST);
        expect(response.body.message).toBe("The paragraph is the beginning of a story and thus cannot be deleted");
    });
    ...
    test('Test of failed delete paragraph (paragraph has choices)', async () => {
        ...
        const story = await createStory('3. Test of failed delete paragraph (paragraph has choices)');
        const token = await getToken();
        ...
        response = await request(app)
            ...
            .send(
                JSON.stringify({
                    titreChoix: '3. Test of failed delete paragraph (paragraph has choices)',
                    idParagraphe: story.idParagrapheInitial,
                    idChoix: null
                })
            );
        ...
        response = await request(app)
            .delete(
                `/api/histoire/${story.id}/paragraphe/${idChoice1}`
            )
            .set('Content-Type', 'application/json')
            .set('x-access-token', token);
        expect(response.statusCode).toBe(status.BAD_REQUEST);
        expect(response.body.message).toBe("The paragraph has choices and thus cannot be deleted");
    });
});
